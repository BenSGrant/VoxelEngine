...
public class DisplayManager {
	...
	private static final int FPS_CAP = 255;
	...
	public static void createDisplay() {
		...
		try {
			...
			Display.create(new PixelFormat().withDepthBits(24), attribs); // put 8,8,0,8 in new PixelFOrmat for basic
																			// anti-aliasing
			Display.setTitle("GaryTheBlobfish::VoxelEngine v0.3");
	...
	public static void updateDisplay() {
		...
		System.out.println(fps + " FPS    Delta: " + delta + "    Total Existing Vertex Count (not neccessarily rendered): " + TerrainGenerator.getTotalVertexCount());
...
//				
				...
				if (Keyboard.isKeyDown(Keyboard.KEY_ESCAPE) && Mouse.isGrabbed()) { // If e is pressed then mouse is
																					// ungrabbed, mouse can move around
																					// without moving camera later on in
																					// game development
					Mouse.setGrabbed(false);
