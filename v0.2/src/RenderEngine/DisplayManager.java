...
public class DisplayManager {
	...
	public static void createDisplay() {
		...
		Display.create(new PixelFormat().withDepthBits(24), attribs); // put 8,8,0,8 in new PixelFOrmat for basic anti-aliasing
		Display.setTitle("VoxelEngine v0.2");
		//Display.setFullscreen(true);//comment out display mode line for this line to work
...
//				}
//				
				if(Keyboard.isKeyDown(Keyboard.KEY_E) && Mouse.isGrabbed()) { //If e is pressed then mouse is ungrabbed, mouse can move around without moving camera later on in game development
					Mouse.setGrabbed(false);
				}else if(Keyboard.isKeyDown(Keyboard.KEY_E) && !Mouse.isGrabbed()) {
					Mouse.setGrabbed(true);
				}
				if(Keyboard.isKeyDown(Keyboard.KEY_ESCAPE) && Mouse.isGrabbed()) { //If e is pressed then mouse is ungrabbed, mouse can move around without moving camera later on in game development
					Mouse.setGrabbed(false);
